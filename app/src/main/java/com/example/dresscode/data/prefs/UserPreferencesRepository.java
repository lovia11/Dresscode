...
public class UserPreferencesRepository {
    ...
    private final SharedPreferences.OnSharedPreferenceChangeListener listener = (p, key) -> {
        if (KEY_GENDER.equals(key)) {
            genderLiveData.postValue(p.getString(KEY_GENDER, GENDER_UNSET));
        }
    };

    public UserPreferencesRepository(Context context) {
        prefs = context.getApplicationContext().getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
        genderLiveData.setValue(prefs.getString(KEY_GENDER, GENDER_UNSET));
    ...
    public String getGender() {
        String value = prefs.getString(KEY_GENDER, GENDER_UNSET);
        return value == null ? GENDER_UNSET : value;
    }

    public void setGender(String gender) {
        prefs.edit().putString(KEY_GENDER, gender == null ? GENDER_UNSET : gender).apply();
    }
